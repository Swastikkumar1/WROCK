...
Tuning (constants below):
  ...
  WROCK_WELCOME_* — TTS after the song (ElevenLabs). Configure via environment or a `.env`
    file next to this script (ELEVENLABS_API_KEY, ELEVENLABS_VOICE_ID, etc.).
    With WROCK_WELCOME_CACHE_ENABLED, audio is saved under `.cache/wrock_welcome/` (WAV) and
    replayed when phrase + voice + model + format match—no repeat API call. Delete that folder
    or set WROCK_WELCOME_CACHE_ENABLED=False to force a fresh fetch.
...
def _choose_input_device(blocksize: int) -> int:
    ...
    override = (os.environ.get("WROCK_INPUT_DEVICE") or "").strip()
    if override:
        try:
            idx = _resolve_input_device_index(override)
        except ValueError as e:
            log.error("%s", e)
            log.error("Set WROCK_INPUT_DEVICE to a device index or name substring.")
            raise SystemExit(1) from e
        name = sd.query_devices(idx)["name"]
        peak = _probe_input_max_rms(idx, blocksize)
        log.info("Using WROCK_INPUT_DEVICE [%d]: %s", idx, name)
        if peak is None:
            ...
            log.warning(
                ...
                "Check Windows input level or try another WROCK_INPUT_DEVICE.",
                peak,
            )
        else:
...
def _wrock_welcome_cache_dir() -> Path:
    base = Path(__file__).resolve().parent
    override = (os.environ.get("WROCK_WELCOME_CACHE_DIR") or "").strip()
    if override:
        return Path(override).expanduser().resolve()
    return base / ".cache" / "wrock_welcome"


def _wrock_welcome_cache_path(
...
def open_custom_actions() -> None:
    """Open any custom URL or local app configured by the user in .env."""
    custom_url = (os.environ.get("CUSTOM_URL") or "").strip()
    if custom_url:
        try:
            if sys.platform == "win32":
                os.startfile(custom_url)
            else:
                webbrowser.open(custom_url)
            log.info("Opened custom URL: %s", custom_url)
        except Exception as e:
            log.warning("Could not open CUSTOM_URL %r: %s", custom_url, e)

    custom_app = (os.environ.get("CUSTOM_APP_PATH") or "").strip()
    if custom_app:
        try:
            subprocess.Popen([custom_app])
            log.info("Launched custom app: %s", custom_app)
        except Exception as e:
            log.warning("Could not launch CUSTOM_APP_PATH %r: %s", custom_app, e)
    ...
    try:
        with sd.InputStream(
            ...
            while True:
                ...
                # Expire old single clap if user waited too long
                ...
                if (
                    ...
                    if first_clap_time is None:
                        first_clap_time = now
                        log.info(
                            "👏 [Clap 1/2 Detected!] (rms=%.4f, threshold=%.4f) — Clap again within %.2fs!",
                            level,
                            threshold,
                            MAX_DOUBLE_GAP_S,
                        )
                        ...
                        if gap < MIN_DOUBLE_GAP_S:
                            ...
                            log.info(
                                "💥 [Clap 2/2 Double Clap Verified!] (gap=%.3fs, rms=%.4f) — Running W.R.O.C.K. sequence!",
                                gap,
                                level,
                            )
                            threading.Thread(
