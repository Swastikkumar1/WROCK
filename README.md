# Desktop clap → W.R.O.C.K. (Worlds Reset On Command, King)

Python script that listens to your default microphone and runs a **double-clap** welcome flow (Spotify, Chrome windows, ElevenLabs voice, Cursor). See constants at the top of `wrock.py` for behavior and tuning.
...
The script loads a **`.env` file** in the same folder as `wrock.py` (via `python-dotenv`). You can also set variables in the shell.
...
| `WROCK_WELCOME_CACHE_DIR` | Custom folder for cached welcome WAV (default: `.cache/wrock_welcome/` under the project). |
| `WROCK_INPUT_DEVICE` | Optional mic override: **integer** index or **substring** of the device name. If unset, the script uses the Windows default; when that mic is silent, it auto-picks the loudest working input. List devices: `python -c "import sounddevice as sd; print(sd.query_devices())"`. |
...
Edit the constants at the top of `wrock.py`:
...
- **Wrong or quiet mic:** On startup the script probes your default Windows input. If it is silent, it **auto-selects** the loudest working mic. To force a specific device, set `WROCK_INPUT_DEVICE` in `.env` (index or name substring from `sounddevice.query_devices()`).
